...
export default function Home() {
  return (
    <div className="w-full mx-auto pr-4 mr-8 flex items-start justify-center from-slate-50 to-white pt-8 pb-10">
      <div className="w-full max-w-full bg-white rounded-3xl shadow-2xl p-10 flex flex-col items-center border border-slate-100">
        <h1 className="text-4xl font-extrabold mb-2 text-blue-700 tracking-tight text-center pb-8">Bienvenido a PibuFarma</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl mx-auto">
          {options.map((opt) => (
            <Link
              key={opt.href}
              href={opt.href}
              className="flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 rounded-2xl shadow hover:shadow-lg transition-all duration-200 p-6 group hover:scale-105 focus:outline-none min-h-[150px]"
            >
              {/* Aseguramos que el contenido dentro de la tarjeta esté centrado vertical y horizontalmente */}
