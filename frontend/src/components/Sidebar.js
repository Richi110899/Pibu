  ...
  return (
    <aside className="h-screen w-64 bg-gradient-to-b from-slate-50 to-white border-r border-slate-200 flex flex-col fixed left-0 top-0 z-20 shadow-xl">
      ...
      <div className="px-6 py-4">
        {session ? (
          <div>
            <p className="text-sm font-semibold pb-1">Hola, {session.user?.name || session.user?.email}</p>
            <p className="text-xs">Usuario autenticado</p>
          </div>
        ) : (
          <p className="text-sm font-semibold">No estás logueado</p>
        )}
      </div>
      ...
      <div className="p-4 border-t border-slate-200 mt-auto">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-red-200 text-red-700 font-semibold hover:bg-red-100 hover:shadow transition-all duration-200 focus:outline-none"
        >
